...
# Path to Voxceleb I.
VOXCELEB_1_DIR=/share/mini5/data/audvis/dia/voxceleb/voxceleb1
VOXCELEB_2_DIR=/share/mini5/data/audvis/dia/voxceleb/voxceleb2

nnet_dir=exp/xvector_nnet_1a

dihard_dev=dihard_dev_2018
voxceleb1=voxceleb1_full
voxceleb2=voxceleb2_full
...
if [ $stage -le 0 ]; then
  ...
  # Prepare data directory for Voxceleb 1.
  echo "Preparing data directory for Voxceleb1..."
  VOX_1_DATA_DIR=data/${voxceleb1}
  rm -fr $VOX_1_DATA_DIR
  local/make_voxceleb1.pl \
     $VOXCELEB_1_DIR data/
  utils/fix_data_dir.sh $VOX_1_DATA_DIR

  # Prepare data directory for Voxceleb 2.
  echo "Preparing data directory for Voxceleb2..."
  VOX_2_DATA_DIR=data/${voxceleb2}
  rm -fr $VOX_2_DATA_DIR
  local/make_voxceleb2.pl \
     $VOXCELEB_2_DIR data/
  utils/fix_data_dir.sh $VOX_2_DATA_DIR
fi
echo "Stage 0: Prepare data directory done."

if [ $stage -le 1 ]; then
  # Make MFCCs for all data dirs.
  ...
  steps/make_mfcc.sh --write-utt2num-frames true \
    --mfcc-config conf/mfcc.conf --nj 40 --cmd "$train_cmd" \
    ${VOX_1_DATA_DIR} exp/make_mfcc $mfccdir
  utils/fix_data_dir.sh ${VOX_1_DATA_DIR}

  steps/make_mfcc.sh --write-utt2num-frames true \
    --mfcc-config conf/mfcc.conf --nj 40 --cmd "$train_cmd" \
    ${VOX_2_DATA_DIR} exp/make_mfcc $mfccdir
  utils/fix_data_dir.sh ${VOX_2_DATA_DIR}

  # Compute VAD separately for all, because we already have gold speech
  ...
  sid/compute_vad_decision.sh --nj 40 --cmd "$train_cmd" \
    ...
    ${VOX_1_DATA_DIR} exp/make_vad $vaddir
  utils/fix_data_dir.sh ${VOX_1_DATA_DIR}

  sid/compute_vad_decision.sh --nj 40 --cmd "$train_cmd" \
    --vad-config conf/vad.conf \
    ${VOX_2_DATA_DIR} exp/make_vad $vaddir
  utils/fix_data_dir.sh ${VOX_2_DATA_DIR}

  # Combine both.
  utils/combine_data.sh data/train_with_sil $DEV_DATA_DIR $VOX_1_DATA_DIR $VOX_2_DATA_DIR
fi
